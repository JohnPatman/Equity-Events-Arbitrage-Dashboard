...
def load_earnings(ticker: str):
    ...
    Cleans:
      - datetime with mixed timezones
      - numeric fields
      - missing surprise values
    Computes:
      - next earnings date
      - next EPS estimate
      - beat rate, avg surprise, stdev
    """
    ...
    if not os.path.exists(path):
        print(f"[ERROR] Missing CSV: {path}")
        return None, None
    ...
    # -------------------------
    # Parse timezone-aware datetimes then drop timezone safely
