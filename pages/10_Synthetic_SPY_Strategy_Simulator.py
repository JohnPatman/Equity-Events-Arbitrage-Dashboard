...
# ============================
# Bloomberg matplotlib style
# ============================
plt.rcParams.update({
        "figure.facecolor": "#000000",
        "axes.facecolor": "#0A0A00",
        "axes.edgecolor": "#FF8C00",
        "axes.labelcolor": "#FF8C00",
        "xtick.color": "#FF8C00",
        "ytick.color": "#FF8C00",
        "grid.color": "#1A1A00",
        "grid.linestyle": "--",
        "grid.alpha": 0.5,
        "text.color": "#FF8C00",
        "legend.facecolor": "#0A0A00",
        "legend.edgecolor": "#FF8C00",
        "legend.labelcolor": "#FF8C00",
        "savefig.facecolor": "#000000",
        "lines.color": "#FF8C00",
})
CHART_COLORS = ["#FF8C00", "#00FF41", "#FFB347", "#00BFFF"]
...
DEFAULT_TABLE_SPY_PRICE = 689.56
...
with st.expander("Adjust Assumptions", expanded=True):
    ...
    with colA:
                roll_cost_bps = st.slider(
                                ...
                                5.0,
                                0.5,
                                help="Applied at each roll date as a cost: Notional x (bps/10,000).",
                )
            with colB:
...
with st.expander("Option net-debit curve (editable)", expanded=False):
        st.caption(
                    ...
                    "We convert $ debits -> % of notional at that reference price, then annualise."
        )
    ...
    table_spy_price = st.number_input(
                ...
                step=0.01,
                help="Important: your $ net debits depend on spot level.",
    )
...
def _roll_dates(index: pd.DatetimeIndex, roll_months: int) -> list:
        if index.empty:
                    return []
...
def _annualised_carry_from_table(roll_months, net_debit_table_months, table_spy_price, contracts, multiplier=100):
    ...
    for mths, debit in sorted(net_debit_table_months.items()):
        ...
        rows.append({"Months": int(mths), "Years": T, "NetDebit_$": float(debit), "Debit_%Notional": debit_pct, "AnnualCarry": annual_carry})
...
def load_spy(start_date: dt.date, end_date: dt.date) -> pd.DataFrame:
    ...
    df = _yf_download_retry("SPY", start_date=start_date, end_date=end_date, attempts=3)
    if df.empty:
...
def load_irx(start_date: dt.date, end_date: dt.date) -> pd.Series:
        end_date = _clamp_end_date(end_date)
    df = _yf_download_retry("^IRX", start_date=start_date, end_date=end_date, attempts=3)
    if df.empty:
...
def load_benchmark_adjclose(ticker: str, start_date: dt.date, end_date: dt.date) -> pd.Series:
        end_date = _clamp_end_date(end_date)
    df = _yf_download_retry(ticker, start_date=start_date, end_date=end_date, attempts=3)
    if df.empty:
...
if run:
    ...
    if init_cash_f < float(est_initial_margin):
        ...
        st.warning(
                        f"Initial margin feasibility check (entry at {start_dt.date()})\n\n"
    ...
    if use_net_debit_carry:
                carry_r_annual, carry_curve = _annualised_carry_from_table(
                    ...
                    table_spy_price=float(table_spy_price),
                    contracts=int(contracts),
    ...
    axp.plot(prices.index, prices["Close"], color=CHART_COLORS[0], linewidth=1.2)
    ...
    if roll_bps > 0:
        ...
        st.caption(f"Roll friction: {roll_bps:.1f} bps per roll | Roll events: {len(roll_days)} | Total roll cost: ${total_roll_cost:,.0f}")
    ...
    ax.plot(res.index, res["Synthetic_Equity_Net"], label="Synthetic (net)", color=CHART_COLORS[0], linewidth=1.5)
    ax.plot(spy_bh_eq.index, spy_bh_eq.values, label="SPY Buy & Hold", color=CHART_COLORS[1], linewidth=1.2)
    if sso_bh_eq.notna().any():
                ax.plot(sso_bh_eq.index, sso_bh_eq.values, label="SSO Buy & Hold (2x)", color=CHART_COLORS[2], linewidth=1.2)
    if upro_bh_eq.notna().any():
                ax.plot(upro_bh_eq.index, upro_bh_eq.values, label="UPRO Buy & Hold (3x)", color=CHART_COLORS[3], linewidth=1.2)
    ax.legend()
    ax.grid(True)
    st.pyplot(fig)
    plt.close(fig)
    ...
    ax2.plot(res.index, res["Synthetic_Notional"], label="Notional", color=CHART_COLORS[0], linewidth=1.2)
    ax2.plot(res.index, res["Margin_Req"], label="Margin Req", color=CHART_COLORS[1], linewidth=1.2)
    ax2.legend()
    ax2.grid(True)
    st.pyplot(fig2)
    plt.close(fig2)
