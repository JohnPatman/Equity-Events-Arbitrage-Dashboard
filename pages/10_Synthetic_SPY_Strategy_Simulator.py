...
BLOOMBERG_MPL = {
        ...
        "figure.edgecolor": "#FF8C00",
        "savefig.facecolor": "#000000",
        "lines.color": "#FF8C00",
}

def apply_bloomberg_mpl(fig, ax_list=None):
        """Apply Bloomberg terminal colours to a matplotlib figure."""
        fig.patch.set_facecolor("#000000")
        axes = ax_list if ax_list is not None else fig.axes
        for ax in (axes if hasattr(axes, "__iter__") else [axes]):
                    ax.set_facecolor("#0A0A00")
                    ...
                    if ax.get_title():
                                    ax.title.set_color("#FFB347")
                                legend = ax.get_legend()
                    if legend:
                                    legend.get_frame().set_facecolor("#0A0A00")
                                    legend.get_frame().set_edgecolor("#FF8C00")
                                    for text in legend.get_texts():
                                                        text.set_color("#FF8C00")

                        CHART_COLORS = ["#FF8C00", "#00FF41", "#FFB347", "#00BFFF", "#FF4500"]
